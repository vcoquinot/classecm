...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Avoir au passé composé</title>
    </head>

    <body> 
        <div class="container">
            <header class="d-flex align-items-center flex-column">
                <h1>Avoir au passé composé</h1>
            </header>

            <main class="quizz text-center mb-4 pt-2 pb-2 bg-info">
            <h2 class="play text-light">À TOI DE JOUER</h2>
            <p class="subheading font-weight-light mb-2 text-light">Conjugue avoir au passé composé</p>
            <!-- Image -->
            ...
            <?php
            //BDD random question
            $question = $bdd->query("SELECT intitule_question, id_question FROM question WHERE id_matiere = 2 && id_theme = 11 ORDER BY RAND() LIMIT 1");
            ...
            <div class="form-group">
                <form action="cible_conjugaison.php" method="get">        
                    ...
                    <!--Form answer-->
                    <?php conjugationForm();?>
                    <input class="input border border-primary" type="hidden" name="url" value="conjugaison_passecompose_avoir.php">
                </form>
            </div>
        </main>
    </body>
